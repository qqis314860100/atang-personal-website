...
export function useScrollLock(options: UseScrollLockOptions = {}) {
  const {
    enabled = true,
    selector = 'div, section, article, aside, main, nav, header, footer, ul, ol, li, table, form',
  ...
  const lockScroll = useCallback(() => {
    ...
    document.body.style.top = preserveScrollPosition
      ? `-${scrollPositionRef.current.y}px`
      : '0'
    document.body.style.left = preserveScrollPosition
      ? `-${scrollPositionRef.current.x}px`
      : '0'
    ...
    // 保存滚动位置到body数据属性
    if (preserveScrollPosition) {
      document.body.dataset.scrollY = scrollPositionRef.current.y.toString()
      document.body.dataset.scrollX = scrollPositionRef.current.x.toString()
    }
  ...
  }, [enabled, selector, preserveScrollPosition])
